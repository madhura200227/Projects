...
# Handle aioredis import (Python 3.13 compatibility)
try:
    import aioredis
except (ImportError, TypeError) as e:
    logging.warning(f"aioredis import failed ({e}), using in-memory fallback only")
    aioredis = None
...
async def init_redis() -> Any:
    global _redis
    if aioredis is None:
        logger.warning("⚠️  aioredis not available. Using in-memory fallback.")
