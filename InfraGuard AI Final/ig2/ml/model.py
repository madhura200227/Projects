...
# Mock numpy if not available (for environments without build tools)
try:
    import numpy as np
except ImportError:
    logging.warning("NumPy not available – using mock for model inference")
    class MockNumpy:
        class random:
            @staticmethod
        ...
        def std(x):
            if isinstance(x, list):
                ...
                return (sum((v - mean_val) ** 2 for v in x) / len(x)) ** 0.5 if x else 0
            return 0
        @staticmethod
        def array(x):
            return x
...
class CNNImageModel:
    def __init__(self, damage_model_path: str, infra_type_model_path: Optional[str] = None):
        self.damage_model_path = damage_model_path
        self.infra_type_model_path = infra_type_model_path
        self.damage_model = None
        self.infra_type_model = None
        self.loaded_ok = False
        self.infra_type_loaded_ok = False

    def load(self) -> None:
        # Damage model (multi-label)
        self.loaded_ok = False
        self.damage_model = None
        if Path(self.damage_model_path).exists():
            try:
                import torch
                self.damage_model = torch.load(self.damage_model_path, map_location="cpu")
                self.damage_model.eval()
                self.loaded_ok = True
                logger.info("✅ CNN damage model loaded")
            except Exception as e:
                logger.warning(f"CNN damage model load failed: {e}")
        if not self.loaded_ok:
            logger.info("CNN damage weights not found – using image statistics heuristic")

        # Infra-type classifier (optional)
        self.infra_type_loaded_ok = False
        self.infra_type_model = None
        if self.infra_type_model_path and Path(self.infra_type_model_path).exists():
            try:
                import torch
                self.infra_type_model = torch.load(self.infra_type_model_path, map_location="cpu")
                self.infra_type_model.eval()
                self.infra_type_loaded_ok = True
                logger.info("✅ Infra-type classifier model loaded")
            except Exception as e:
                logger.warning(f"Infra-type model load failed: {e}")
    ...
    def predict_from_array(self, img_array: np.ndarray) -> Dict[str, float]:
        ...
        # Laplacian sharpness → edges / cracks (skip scipy if unavailable)
        try:
    ...
    def classify_infra_type(self, img_array: np.ndarray) -> Dict[str, Any]:
        ...
        Fallback: deterministic low-confidence default.
        """
        # If we have a trained infra-type classifier, use it.
        if self.infra_type_model is not None:
            try:
                import torch
                from torchvision import transforms
                from PIL import Image

                classes = ["road", "bridge", "pipeline", "building", "manhole"]
                pil = Image.fromarray(img_array.astype(np.uint8))
                transform = transforms.Compose([
                    transforms.Resize((224, 224)),
                    transforms.ToTensor(),
...
class InfraGuardModel:
    ...
    def __init__(self):
        ...
        self.cnn       = CNNImageModel(settings.CNN_MODEL_PATH, settings.INFRA_TYPE_MODEL_PATH)
