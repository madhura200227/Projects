...
async def geocode_location(lat: float, lon: float) -> str:
    ...
    Falls back to nearest_city if geocoding fails.
    """
    try:
        async with _client(timeout=5) as c:
            r = await c.get(
                ...
                headers={"User-Agent": "InfraGuardAI/2.0 (+https://infraguard.ai)"}
            )
            r.raise_for_status()
            data = r.json()
    ...
    # Fallback to nearest hardcoded city
    nearest = nearest_city(lat, lon)
    logger.info(f"Geocoding fallback: using nearest city {nearest}")
    return nearest
...
class DataAggregator:
    ...
    async def fetch_all(
        ...
        # Geocode the location to get actual city name
        geocoded_city = await geocode_location(lat, lon)
        
        # Check if it's a known city
        if geocoded_city in CITY_PROFILES:
            baseline = dict(CITY_PROFILES[geocoded_city])
            city = geocoded_city
        else:
            # For unknown cities, use a generic baseline instead of forcing nearest city
            baseline = {
                "avg_daily_traffic": 50_000,  # Use median estimate
                "heavy_vehicle_pct": 20,      # Default estimate
                "aqi": 100,                   # Moderate
                "flood_events_5yr": 5,        # Generic estimate
                "road_age_years": 15,         # Average
                "rainfall_mm_annual": 1000,   # Global average
                "soil_moisture": 50,          # Moderate
                "surface_material_score": 0.7,
                "drainage_quality_score": 0.7,
                "image_damage_score": 0,
                "population_density": 3000,
                "temperature_extremes": 30,
                "seismic_zone": 2,
                "congestion_pct": 40,
                "nearby_waterways": 0,
            }
            city = geocoded_city or nearest_city(lat, lon)
        ...
        sources_used = {"baseline": "generic" if geocoded_city not in CITY_PROFILES else city}
