    ...
    if collect and settings.DATA_COLLECTION_DIR:
        try:
            collected = persist_image_sample(
                ...
                metadata={"endpoint": "/api/v1/predict/image"},
                subdir="predict_image",
                image_ext=".jpg",
            )
        except Exception as e:
            logger.warning(f"Data collection failed (predict/image): {e}")

    # Classify infrastructure type
    TYPE_ALIASES = {"roads": "road", "bridges": "bridge", "pipelines": "pipeline"}
    if infra_type:
        infra_type = TYPE_ALIASES.get(infra_type.strip().lower(), infra_type.strip().lower())
    classification = model.cnn.classify_infra_type(img_arr)
    detected_type  = infra_type or classification["detected_type"]
    confidence     = classification["confidence"]
    method         = classification.get("method", "model" if confidence > 0 else "fallback")

    # Reject non-infrastructure images (low confidence = wrong type)
    VALID_TYPES = {"road", "bridge", "pipeline", "building", "manhole"}
    # Only enforce rejection if we have a real classifier.
    if method == "model" and (detected_type not in VALID_TYPES or confidence < 0.55):
